...
export default function MatchStatsPanel({ clientId }: { clientId: string }) {
  ...
  return (
    <div>
      ...
      {matches.length > 0 && (
        <div>
          <div style={{ fontSize: 12, color: "var(--text-dim)", marginBottom: 6 }}>Recent matches</div>
          {matches.map((m) => (
            <div key={m.matchId} className="skill-row">
              ...
              <div className="skill-score" style={{ fontSize: 10 }}>
                {m.finishedAt ? new Date(m.finishedAt).toLocaleDateString() : ""}
              </div>
            </div>
          ))}
        </div>
